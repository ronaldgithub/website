...
Bilingual (NL/EN) static portfolio site for Ronald de Groot, senior (Azure) SQL Server DBA — served at dbaronald.com. Plain HTML/CSS/JS: **no build step, no framework, no package.json, no tests**. Hosted on Hostinger shared hosting (Apache/LiteSpeed). The companion domain dbaronald.nl runs a separate WordPress blog (see "Adding a blog article") and, since 2026-08-19, three interactive PHP-backed features hand-installed as WPCode snippets: a public **Statistics Parser** community tool, a public **Execution Plan Analyzer** community tool, and a private **execution plan submission form** (see their respective sections below) — all three share their client-side JS engines with this static repo but are otherwise entirely outside the normal git-deploy pipeline.
...
## Adding the Execution Plan Analyzer tool

A free community tool at a wp-admin-chosen slug on dbaronald.nl (e.g. `dbaronald.nl/execution-plan-analyzer/`): paste `Show Execution Plan XML` (or upload a `.sqlplan` file), get the operator list plus rule-based red flags (spills to tempdb, implicit conversions, cardinality skew ≥10×, key/RID lookups executed >100×, missing-index suggestions, small parallel plans), plus an optional "email me the full report" button. **Same two-path structure and abuse model as the Statistics Parser** — it's that pattern applied to execution plans, not to be confused with the private *execution plan submission form* below (which forwards a file to Ronald and analyzes nothing):

- `assets/js/execution-plan-analyzer.js` — the parsing/findings engine (pure client-side, `DOMParser` on the showplan XML, no network calls). Lives in the **main static repo**, deploys automatically via `git push`. The live WordPress page loads it straight from `https://dbaronald.com/assets/js/execution-plan-analyzer.js` via a plain cross-origin `<script src>`. Root element is `.plan-analyzer`; all its own classes are `epa-`-prefixed; findings reuse the shared `.pill`/`.pill-accent`/`.pill-amber` component.
- `wordpress/plan-analyzer-page.html` — a standalone copy of the page markup for **local testing only** (`python -m http.server 8123`, open `/wordpress/plan-analyzer-page.html`). Its "email me" button has no real backend behind it.
- `wordpress/execution-plan-analyzer-handler.php` — the real thing: a WPCode "Run Everywhere" PHP snippet (install steps in its header comment) registering the `[dbaronald_plan_analyzer]` shortcode (same fresh-nonce-per-view reasoning as Statistics Parser) and the `wp_ajax_dbaronald_plan_analyzer_email` AJAX handler. The handler re-parses the plan with `DOMDocument`/`DOMXPath` and re-implements the **same rule set as `execution-plan-analyzer.js`** independently in PHP (`dbaronald_plan_analyzer_parse()` mirrors `parsePlan()`, `dbaronald_plan_analyzer_build_findings()` mirrors `buildFindings()`) — never trust client-computed results for something that gets emailed out — then sends via `wp_mail()`. **Keep the two rule sets in sync** when adding/changing a finding.
- Abuse protection on the email path: a WP nonce, a 5-emails/hour-per-IP rate limit (`dbepa_rl_` transients), and Cloudflare Turnstile — same `DBARONALD_TURNSTILE_SITE_KEY`/`DBARONALD_TURNSTILE_SECRET_KEY` `wp-config.php` secrets as Statistics Parser (if that snippet is installed they're already set). Same known accepted risk: the endpoint emails whatever address is typed in.
- Shares the `wp_mail()` deliverability problem noted at the bottom of this file.
- To create the live page: paste `execution-plan-analyzer-handler.php` into a WPCode PHP snippet, create a WP Page whose entire content is `[dbaronald_plan_analyzer]`, then add it to the site nav via Appearance → Menus.
